...
public class DelSt extends javax.swing.JFrame {
    public DelSt() {
        initComponents();
        myconnection();
    }
    ...
    private void SSubmitActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_SSubmitActionPerformed
        ...
        try {
            delSt.setInt(1, vcode);
            delSt.setString(2, vname);
            delSt.setString(3, vsurname);
            delSt.setString(4, vclass);
            delSt.setInt(5, vage);
            if(flag==true){
    ...
    void myconnection () {
        ...
        String deleteString = "{call delSt(?,?,?,?,?)}";
	delSt = dbConnection.prepareCall(deleteString);
        } catch(SQLException ex) {
            System.out.println("\n -- SQL Exception --- \n");
            while(ex != null) {
		System.out.println("Message: " + ex.getMessage());
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(DelSt.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
    ...
    static CallableStatement  delSt = null;
